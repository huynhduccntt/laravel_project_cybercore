<main class="main-content" style="position: relative;   top: -100px; max-width: 1280px !important; ">
  <section class="slideshow">
    <div class="slideshow-inner">
      <div class="slides">
        <div class="slide is-active ">
          <div class="slide-content">
            <div class="caption">
              <div class="title">HỆ THỐNG PHÒNG GAME<br>SỐ 1 VIỆT NAM</div>
              <div class="text">
                <p>DẪN ĐẦU CÔNG NGHỆ, HOÀN HẢO VỀ DỊCH VỤ</p>
              </div>
              <a href="#" class="btn">
                <span class="btn-inner">ĐĂNG KÝ TƯ VẤN PHÒNG GAME</span>
              </a>
            </div>
          </div>
          <div class="image-container">
            <img src="{{URL::asset('frontEnd/images/07/2-1.jpg')}}  " alt="" class="image" />
          </div>
        </div>
        <div class="slide">
          <div class="slide-content">
            <div class="caption">
              <div class="title">CAM KẾT GIÁ TỐT NHẤT</div>
              <div class="text">
                <p>CUNG CẤP NHỮNG DÒNG SẢN PHẨM CHUYÊN GAME ĐỘC QUYỀN</p>
              </div>
              <a href="#" class="btn">
                <span class="btn-inner">ĐĂNG KÝ TƯ VẤN PHÒNG GAME</span>
              </a>
            </div>
          </div>
          <div class="image-container">
            <img src="{{URL::asset('frontEnd/images/07/5.png')}} " alt="" class="image" />
          </div>
        </div>
        <div class="slide">
          <div class="slide-content">
            <div class="caption">
              <div class="title">ĐỘI NGŨ KỸ THUẬT CHUYÊN NGHIỆP</div>
              <div class="text">
                <p>HỖ TRỢ TỪ XA, XỬ LÝ TRỰC TIẾP ĐẢM BẢO QUÁ TRÌNH KINH DOANH KHÔNG BỊ GIÁN ĐOẠN</p>
              </div>
              <a href="#" class="btn">
                <span class="btn-inner">ĐĂNG KÝ TƯ VẤN PHÒNG GAME</span>
              </a>
            </div>
          </div>
          <div class="image-container">
            <img src="{{URL::asset('frontEnd/images/07/6.png')}} " alt="" class="image" />
          </div>
        </div>
        <div class="slide">
          <div class="slide-content">
            <div class="caption">
              <div class="title">VỚI HƠN 10 NĂM KINH NGHIỆM PHÒNG GAME</div>
              <div class="text">
                <p>>CHÚNG TÔI ĐỒNG HÀNH CÙNG BẠN ĐI ĐẾN THÀNH CÔNG</p>
              </div>
              <a href="#" class="btn">
                <span class="btn-inner">ĐĂNG KÝ TƯ VẤN PHÒNG GAME</span>
              </a>
            </div>
          </div>
          <div class="image-container">
            <img src="{{URL::asset('frontEnd/images/07/DSC_0094-7.jpg')}}" alt="" class="image" />
          </div>
        </div>
      </div>
      <div class="pagination">
        <div class="item is-active">
          <span class="icon">1</span>
        </div>
        <div class="item">
          <span class="icon">2</span>
        </div>
        <div class="item">
          <span class="icon">3</span>
        </div>
        <div class="item">
          <span class="icon">4</span>
        </div>
      </div>
      <div class="arrows">
        <div class="arrow prev">
          <span class="svg svg-arrow-left">
            <svg version="1.1" id="svg4-Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="14px" height="26px" viewBox="0 0 14 26" enable-background="new 0 0 14 26" xml:space="preserve">
              <path d="M13,26c-0.256,0-0.512-0.098-0.707-0.293l-12-12c-0.391-0.391-0.391-1.023,0-1.414l12-12c0.391-0.391,1.023-0.391,1.414,0s0.391,1.023,0,1.414L2.414,13l11.293,11.293c0.391,0.391,0.391,1.023,0,1.414C13.512,25.902,13.256,26,13,26z" /> </svg>
            <span class="alt sr-only"></span>
          </span>
        </div>
        <div class="arrow next">
          <span class="svg svg-arrow-right">
            <svg version="1.1" id="svg5-Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="14px" height="26px" viewBox="0 0 14 26" enable-background="new 0 0 14 26" xml:space="preserve">
              <path d="M1,0c0.256,0,0.512,0.098,0.707,0.293l12,12c0.391,0.391,0.391,1.023,0,1.414l-12,12c-0.391,0.391-1.023,0.391-1.414,0s-0.391-1.023,0-1.414L11.586,13L0.293,1.707c-0.391-0.391-0.391-1.023,0-1.414C0.488,0.098,0.744,0,1,0z" /> </svg>
            <span class="alt sr-only"></span>
          </span>
        </div>
      </div>
    </div>
  </section>
</main>